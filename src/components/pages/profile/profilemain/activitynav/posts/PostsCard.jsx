...
import { deletePost, fetchCurrentUserPosts, fetchCurrentPostComments, handleLike, addComment} from "../../../../../../store";
...
//     }
    ...
    function PostsCard({deletePost, fetchCurrentPostComments, handleLike, addComment}) {
        ...
        <div>
            {currentUserPosts?.slice(0)?.reverse()?.map((post, key) =>(
                <div className=" answers-card border-bottom" key={key}>
                    <div className="d-flex flex-column p-2 align-items-start" style={{width: "100%"}}>
                        <div className="my-2">
                            <h5>{post?.title}</h5>
                        ...
                        <div className="d-flex align-items-center justify-content-between w-100">
                            <div className="d-flex align-items-center">
                                <div className="d-flex align-items-center answers-reaction px-3 py-1 justify-content-center mr-2">
                                    <i className="far fa-thumbs-up pr-2 mr-2 border-right" onClick={() => handleLikeTest(post?._id)}><span>{post?.likes?.length}</span></i>
                                    <i className="far fa-thumbs-down"></i>
                                </div>
                                <i className="far fa-comment p-2" 
                                onClick={()=> {fetchCurrentPostComments(post?._id); setTimeout(() => {
                        ...
                        <div className="comment-area w-100" onClick={()=> fetchCurrentPostComments(post?._id)}>
                            ...
                            <PostCommentsList postId={post?._id} currentUser={currentUser} setCommentContent={setCommentContent} commentContent={commentContent} fetchCurrentPostComments={fetchCurrentPostComments} addComment={addComment} showLoadMoreBtn={showLoadMoreBtn} setShowLoadMoreBtn={setShowLoadMoreBtn}/>
                        </div>
